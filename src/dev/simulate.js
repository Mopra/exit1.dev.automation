...
// without ever posting to X. Verifies normalization, dedup, threaded
// recovery, flap suppression, the budget guard, AND status-page link
// resolution (a real public-monitored site -> /status/<slug>; an unknown
// site -> home page).
//
//   npm run simulate
//
...
const steps = [
  [`public site down (${publicSite.host} → status page)`, down(publicSite, base - minutes(42))],
  ['same site down again (duplicate)', down(publicSite, base - minutes(40))],
  ['public site recovers (threaded reply, ~42m)', up(publicSite, base)],
  [`unknown site down (${privateSite.host} → home fallback)`, down(privateSite, base - minutes(3), { code: 500 })],
  ['unknown site recovers quickly (flap)', up(privateSite, base - minutes(1))],
  ['unknown site flaps down again (cooldown should suppress)', down(privateSite, base, { code: 500 })],
];
